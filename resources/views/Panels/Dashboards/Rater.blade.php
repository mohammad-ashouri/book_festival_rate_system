...
@section('content')
    <main class="flex-1 bg-cu-light py-6 px-8">
        ...
        @elseif( count($formalLiteraryRates) > 0 )
            <div class="mx-auto lg:mr-72">
                <h1 class="text-2xl font-bold mb-4">فهرست آثار در حال ارزیابی ادبی صوری</h1>
                <div class="bg-white rounded shadow p-6">
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center">
                        <thead>
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                        ...
                        <tbody class="divide-y divide-gray-300">
                        @foreach ($formalLiteraryRates as $rate)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration  }}</td>
                                <td class="px-6 py-4">{{ $rate->postInfo->title  }}</td>
                                <td class="px-6 py-4">{{ $rate->postInfo->post_format  }}</td>
                                ...
                                <td class="px-6 py-4">
                                    <a href="/Rate/FormalLiterary/{{ $rate->id }}">
                                        <button type="button"
                                                class="px-4 py-2 mr-3 bg-green-500 text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring focus:border-blue-300">
                                            ثبت ارزیابی
                                        </button>
