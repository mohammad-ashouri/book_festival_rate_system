...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">ویرایش ناشر</h1>
            @if (count($errors) > 0)
            ...
            <div class="bg-white rounded shadow flex flex-col ">
                {!! Form::model($publisher, ['method' => 'PATCH','id'=>'edit-publisher','route' => ['Publishers.update', $publisher->id]]) !!}
                @csrf
                <div class="flex bg-white px-4 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="flex-1 flex-col items-right ml-3">
                        ...
                        <select id="status"
                                ...
                                name="status">
                            <option @if($publisher->status==1) selected @endif value="1">فعال</option>
                            <option @if($publisher->status==0) selected @endif value="0">غیرفعال</option>
                        </select>
                    </div>
                </div>
